...
| Fix applied | (1) `apps/web/app/lib/api-client.ts`: extended `BattleApiError` with optional `body: unknown` field (backward-compatible, 4th constructor arg defaults to null); new `retryBattlePool(battleId)` + `PoolRetryResponse` type mirroring the backend discriminated union. (2) `apps/web/app/routes/_app.battle.pre.$id.tsx`: new `handleRetryPool` useCallback with 200/202/409/4xx-5xx response discriminator; StuckPane props extended with `canRetry` + `retrying` + `onRetry`; host-only third CTA rendered between Cancel and Keep-waiting; all three CTAs share `disabled={cancelling || retrying}`; Sonner toasts surface all three response branches. |
| Fix commits | `8ed3f95` (Task 1 api-client: BattleApiError body + retryBattlePool + PoolRetryResponse), `139825f` (Task 2 StuckPane retry CTA + handler), plus this VALIDATION commit. |
| Verified (this plan) | `cd apps/web && npx tsc -b` exits 0; `npm test -- tests/battle/` remains green (36 files / 137 tests, no backend regression); manual UAT for 04-42 is explicitly out of scope for this plan's automated gate per Nyquist 8a. |
...
| Fix commits | `eaff2ac` (Task 1 battle.room.pool-timeout.test.ts), `fde37af` (Task 2 battle.pool.retry.test.ts), `b851286` (Task 3 battle.workflow.started-at.test.ts), plus this VALIDATION commit (Task 4). |
